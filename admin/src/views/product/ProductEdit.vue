...
import { productUpdataService, productgetItemService } from '@/api/product'
import { useRoute } from 'vue-router'
const ruleForm = ref({})
const route = useRoute()
const productgetItem = async () => {
  const result = await productgetItemService({ _id: route.params.id })
  ruleForm.value = result.data.data
  imageUrl.value = ruleForm.value.productPic
}
productgetItem()
const rules = {
...
const submit = async () => {
  ...
  await productUpdataService(formData)
